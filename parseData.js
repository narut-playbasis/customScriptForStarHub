...
var parameter_name = 'item_id';
var type = process.env.PARAM_TYPE;
var storeOrgName = "Store";
...
for(i in players_file) {
    ...
    for(j in line_array) {
        ...
        if (sub_array[0] == "id"){
            ...
            organize_name_array.push(sub_array[1]);
        }
...
//}
function getManagerPlayerIdByStoreName (store_name){
    for (n in store_id_array){
        if (store_name == store_name_array[n] && organize_name_array[n] != storeOrgName){
            return cl_player_id_array[n];
        }
    }
    return null;
}
function getStorePlayerIdByStoreName (store_name){
    for (n in store_id_array){
        if (store_name == store_name_array[n] && organize_name_array[n] == storeOrgName){
            return cl_player_id_array[n];
        }
...
function findParentsByPlayerId(player_id){
    ...
    if (parent_name != null){
        var parent_player_id = getManagerPlayerIdByStoreName(parent_name);
...
function readLine(index){
    ...
    for(j in input_line) {
        ...
        if (sub_array[0] == "store"){
            ...
            player_id = getStorePlayerIdByStoreName(store_name);
